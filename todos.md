...
- [x] Missing-countries content shipped Sep 13 (`1c1879d`): added 32 creatures for 6 country coverage gaps (Isle of Man 6, Micronesia 5, Oman 5, Korea 6, Shetland 4, Polynesia 6) — researched+authored via 6 parallel subagents, authored to `archive/scripts/gap-creatures/<iso3>.json` then merged by `merge-gap-creatures.cjs` (both gitignored). NOTE São Tomé was NOT a gap (Ossobó already exists under ASCII country `Sao Tome and Principe`; accent-strip matching required). All gates: jest 778/778, eslint 0 err, chromium detail-probe 15/15 new creatures render, 0 console errors. Stale-side gotcha: adding creatures requires rebuilding ALL indexers — `shard-data.mjs` (manifest+region+by-slug) AND `build-{creatures,stories,items,search}-index.py` ([search-index, creatures-index, stories-index, items-index].json) — missing the per-type index builders = 9 failed "slug sets match masters" jest tests.
- [x] ALL PUSHED TO origin/main Sep 13 (29 commits, `20ef8f2..4d6efc9`) + deploy zip REBUILT via `bash archive/scripts/build-deploy.sh` (seo-build.py prerender: sitemap 6536 urls, 14598 files, 102MB → /tmp/whispering-lore-deploy.zip). Remaining is only the stale gitignored stash@{0} (needs manual `git stash drop`).
- [x] es i18n COMPLETE for all Spanish regions Sep 13 (commit `86c03e2` region overlays, PUSHED Sep 13): mesoamerica + caribbean + mediterranean overlays shipped alongside the existing south-america — creatures 144+93+89 FULL, stories 40+72+15 summary-only, items 18+8+38 FULL (total es: creatures 326, stories 127, items 64; 12 manifest i18n keys). Pipeline: EN sources → 24 parallel translation subagents (~25/chunk) → `archive/scripts/i18n/es/gate-overlay.py` generalized (`gate-overlay.py <type> <region>`, per-region chunk glob `<type>-<region>-chunk-*.json`, src `/tmp/es-<region>-<type>-src.json`) → merged 9 overlays `data/i18n/es/*.json` → `shard-data.mjs` manifest rebuild. Gates: jest 778/778, eslint 0 err, browser es-mode probe (bestiary grid es names ×3 regions, stories es titles, items es names; html lang=es, spanish nav, 0 console errors, 0 HTTP ≥400), detail overlay es Spanish body verified. NOTE mediterranean-shard is mixed-language (France 19/Greece 18/Italy 11/Spain 16…): whole-shard es = native-mode promise (Spanish for all covered shard entries); Spain-only would need a non-region-keyed overlay mechanism, not built. Deploy zip REBUILT Sep 13.
- [x] Source-badge duplication removed Sep 13 (`677807b`, PUSHED Sep 13): hero/sidebar source badges now render ONLY when the Sources section stays hidden (source empty or 'None — no independent source located' — 117 creatures match, stories/items 0). `window.__sharedUtils.hasUsableSource(entry)` added to js/shared-utils.js (renderSources reuses it); creatures hero `#detail-source-badge` + `#detail-source-type-badge` gated + cleared (empty-source creature talasac still shows hero 'Poor' badge, section hidden); stories `#detail-source-type-badge` sidebar badge DELETED (js + stories.html), attribution card now shows only the oral_tradition honor text; items sidebar `Source: X` text + `#detail-source-type-badge` DELETED (js + items.html), card shows honor text only (mjolnir literary → card hidden; asson-rattle oral → honor text). `#detail-live-badge` (Living Tradition/Historical) kept — unique to hero. Gates: jest 778 passed (harness mirrors), eslint 0 err, axe detail-overlay 3/3 chromium, browser probe 6/6 checks + 0 console errors (tangaroa-maori usable-source→hero empty, talasac empty-source→hero badge, ragnark oral→card honor text, mjolnir literary→card hidden, asson-rattle oral). 6 files changed (shared-utils, 3 viewers, stories.html, items.html); bestiary.html unchanged (hero spans stay, JS gates). Note: 135 None* variants — only exact sentinel counts as no-source; rest DO render. Deploy zip REBUILT Sep 13.
- [x] Detail heading levels flattened Sep 13 (`a95c05b`, PUSHED Sep 13): detail-view content sections h3→h2 + sidebar cards h4→h2 in bestiary/stories/items.html so ALL content under the entry h1 (#detail-name/#detail-title) is flat h2 (was jumpy: h3 content, h2 cross-ref, h4 sidebar). CSS `.detail-section h3`→`.detail-section h2` + `.cd-sidebar-card h4`→`.cd-sidebar-card h2`. Cross-ref titles (.cd-sidelist-title/.detail-section-title) live in .cd-crossref/.cd-sidelist siblings — no collision. Gates: jest 778 passed (harness mirrors), axe detail-overlay scans 3/3 chromium (heading-order incl.), browser smoke tangaroa-maori/king-lindorm/mjolnir → flat h2 + sources intact (1 list + 2 badges) + 0 console errors. GOTCHAS: perl top-level `shift` empties @ARGV → `<>` reads stdin → 0-byte file; restore via `git cat-file blob <sha> > file` (snip truncates `git show > file`); leftover stash@{0} = corrupted pre-restore copy → `git stash drop` needs user permission
- [x] Detail Sources section SHIPPED Sep 13 (spec docs/superpowers/specs/2026-09-13-detail-sources-section-design.md + plan docs/superpowers/plans/2026-09-13-detail-sources-section.md; commits 7800ad5 + acfc4fd + cd9789f + 67a161a + a2c85a5 + CSS, PUSHED Sep 13): the per-entry `source` string — hidden until now (only source_type/source_quality badges rendered) — now shows in every detail view (bestiary/stories/items) as a "Sources" section after content, before the cross-ref links. Shared `window.__sharedUtils.renderSources(sectionEl, entry)` (js/shared-utils.js): splits source on `;`, auto-links https URLs (target=_blank), renders existing source-type/source-quality badges; stays hidden when source empty or exactly 'None — no independent source located' (135 None* variants exist but only the exact sentinel is empty — all others carry real refs and DO render). 3 static skeletons (bestiary.html detail-creature-sources / stories.html detail-story-sources / items.html detail-item-sources) each with data-i18n="detail.sources" heading; i18n key added en/es/sv/no; viewers call renderSources guarded. CSS .detail-source-list/.detail-source-badges in styles.css. jest 270/270 (23 suites), eslint 0 err. NOTE: creatures hero + items sidebar already show source badges → slight duplication (deliberate). Deploy zip REBUILT Sep 13.
- [x] Audit fixes #1-#4 SHIPPED Sep 13 (plan docs/superpowers/plans/2026-09-13-audit-fixes-secret-and-stories-preload.md; commits 3d30e20 + a78357c + 6c8970b/a85060c + 8dcff9c, PUSHED Sep 13): #1 secret-in-git was a FALSE ALARM (.env never tracked — gitignored + no history) → guard test tests/secret-scan.test.js (no tracked .env, no AIza/sk- patterns); #2 stories.html creatures preload (~29MB full corpus) DELETED + renderDetailCreatures now lazily resolves per creature slug via Shimmer.getItem (probe: 0 by-region, 0 creatures index, 1 by-slug fetch); #3 items slim index data/sharded/items/index.json (641 entries, 687KB; generator archive/scripts/build-items-index.py + tests/items-index.test.js parity) — items grid reads slim index, detail via getItem on _slim, items.html __WL_PRELOAD block (all items + every creature/story ref batch) DELETED, cross-refs now lazy renderRefs/makeRefLink (probe: 1 index fetch, 0 by-region, mjolnir 2 by-slug refs); #4 SearchAction JSON-LD targets fixed search→search.html?q= on 10 pages + robots noindex,follow on 404/search/quiz/mylore. jest 264/264 (23 suites), eslint 0 err. Deploy zip REBUILT Sep 13.
- [x] Stories grid cold-path (Approach B) IMPLEMENTED Sep 12 (plan docs/superpowers/plans/2026-09-12-stories-index-cold-path.md; commits 515aa76 + 50bca3c, sw v1_0_29, PUSHED Sep 13): stories.html grid previously loaded all 139 story region shards (~7.5MB) via loadAllShards; now reads slim data/sharded/stories/index.json (1.74MB, 9 fields/entry, generator archive/scripts/build-stories-index.py + tests/stories-index.test.js parity gate) via Shimmer.loadIndex('stories'); showDetail fetches the full story via getItem on _slim (mirror creatures-viewer). Parity gate relaxed to allow doubled-dash slugs (95 transliteration slugs in master, e.g. mariatta--wainamoinens-departure). jest 256/256 (21 suites), eslint 0 err, sw v1_0_29. Probe probe-stories-index.cjs (untracked): 24 cards, 0 by-region, 1 index fetch, detail renders. Deploy zip REBUILT Sep 13.
- [x] Bestiary detail-view swap IMPLEMENTED Sep 12 (plan docs/superpowers/plans/2026-09-12-bestiary-detail-view-swap.md; commits a226570 spec + 080888b/2154aff/24806a0 code, PUSHED Sep 13): creature detail open previously loaded all 139 story region shards (~7.5MB) via renderCreatureStories→loadAllShards; now reads story `creatures` refs from pre-existing search-index.json with one memoized `Shimmer.getSearchIndexStories()` fetch (retry-on-fail; legacy loadAllShards fallback kept verbatim). Generator archive/scripts/build-search-index.py emits creatures refs; search-index.json 4.1→4.25MB (4,442,853 B). Tests: +1 search-index parity + 2 shimmer loader + 1 retry → jest 248/248 unique / 21 suites (harness mirrors .test.js under /tmp/wl-local/js/ double-collect → 495/41 there), eslint 0. Probe probe-bestiary-index.cjs (untracked): 6 related cards, 0 shard reqs, 1 index fetch. Deploy zip REBUILT Sep 13.
- [x] i18n Phase 2 es (Spanish native mode) SHIPPED Sep 12 (ad107d6 + 5d1486a): chrome dict es 226/226 EXACT parity (4 subagent batches + normalized: space thousands "3 668", source-quality labels kept English), native routing es via getNativeLangs/saved-lang/NATIVE_LANGS/fallback + hreflang ×11 + sw v1_0_28; South America overlays gated → data/i18n/es/ creatures 197 full / stories 17 summary-only / items 12 full (gate-overlay.py: slug parity, length, Cyrillic, lowercase-doubled scan); runtime e2e: es chrome on 4 pages + both es overlays fetched 200; jest 241/241 (20 suites), eslint 0 err; deploy zip rebuilt
...
- [x] i18n Phase 2 Wave B SHIPPED Aug 23 (b38819b..T4): spec docs/superpowers/specs/2026-08-23-i18n-phase2-wave-b-design.md, plan docs/superpowers/plans/2026-08-23-i18n-phase2-wave-b.md — T1 coverage map (b38819b): shard-data.mjs auto-derives manifest.i18n from data/i18n/ dir scan, _loadOverlayFor short-circuits uncovered keys to cached-null (0 i18n 404s per native load, was ~180; fallback fetch when manifest/block absent), fixture manifest + jest spy tests; T2 complete flag (94bf3ab): all three decoration paths compute _i18n.partial=!(patch&&patch.complete), 'complete' key stripped from rendered objects, badge helper unchanged; T3 content batch (978ce2e): 50 FULL swedish entries (positions 51-100, name+summary+description, complete:true) merged into creatures-nordic.json -> 100 total / 50 complete, validation gate enforces slug match + desc >=200 chars; T4 sw v1_0_23 + gates + docs + zip + push
- [x] i18n Phase 2 Wave C SHIPPED Aug 23 (209bedb..T3): spec docs/superpowers/specs/2026-08-23-i18n-phase2-wave-c-design.md, plan docs/superpowers/plans/2026-08-23-i18n-phase2-wave-c.md — T1 stories merge proof (209bedb): fixture sv-stories-nordic + jest test proving loadRegionShard('stories') merge end-to-end, no js changes; T2 content batch (b925b5f): data/i18n/sv/stories-nordic.json = all 66 nordic stories sv title+summary + 10 flagship full_texts complete:true (Kung Lindorm, Fågel Grip, Pojken och trollen, Skatan m salt på svansen, Kópakonan, Risin og Kellingin, Ljungbyhornet, Skutt o Tuvstarr, Trøllanes, Trollridan), coverage map auto-published via shard rebuild; validation gate: slug x66 + full_text >=0.6x EN length; browser-verified flagship badge-free / partial badge / 0 i18n 404s; T3 gates eslint 0 + jest 212/212 + chromium suite + todos + zip + push
- [x] i18n Phase 2 Wave D SHIPPED Aug 23 (cac2c39..T3): spec docs/superpowers/specs/2026-08-23-i18n-phase2-wave-d-design.md + plan wave-d.md — T1 items merge proof (cac2c39, jest 213/213); T2 content batch (a3d1b26) data/i18n/sv/items-nordic.json = all 82 nordic artifacts sv name+description complete:true (gate desc >=0.5x EN), coverage map now sv: creatures+stories+items; browser-verified mjolnir badge-free sv desc / grid sv names / 0 i18n 404s; no js changes -> sw stays v1_0_23
- [x] i18n Phase 2 Wave G SHIPPED Aug 23 (b72bd4c..T3): FINAL nordic batch — G1 entries 201-250 (00d0c53: Dovregubben, De sju systrarna, Huldra, Hafgufa m fl) + G2 entries 251-314 (dfc171c: Väki/Vätt/Vård-familjen, Valravn, Väinämöinen, Näcken/Näkki/Nøkken, Pesta, Draugr-varianter, Beaivi m fl) + pilot completion (Wave A:s 50 entries fick descriptions) -> creatures-nordic.json = 314/314 FULL NORDIC COVERAGE; GOTCHA: glob wg2*.json krockade med G1:s wg2.json, vindra-slug stavat som vindrå, commit-hook kräver korta subjects; gates eslint 0/jest 213/213/chromium 470/470
- [x] i18n Phase 2 Wave H — Celtic creatures SHIPPED (c42c318): junk scaffold files DELETED. `data/i18n/sv/creatures-celtic.json` = 35/35 complete (gate: slug x35, desc >=200 chars, 0 Cyrillic-е). Gates: jest 222/222, i18n e2e 35/35 (2 GT-cookie failures bb6c484 re-scoped to Gemini gmlang state — root: 9a86339 dropped googtrans+element.js, choose() → `__translate.enable`; full chromium 469 pass + 3 load-timeout flakes pass isolated)
- [x] i18n Phase 2 Wave H — Celtic stories SHIPPED (217baed): region-keyed overlays
GATES-CAUGHT-FIX (1b3fc67): sv coverage audit (per-region count vs overlay entries, keyed by loader fileKey) found 1 gap — creature `moon-spirit` (Japan) tagged region "East Asia" (not "East Asia and Pacific"), fell through to English. Added 1-entry `creatures-east-asia.json` overlay. Audit confirms all shipped regions count-exact, 0 dangling slugs. GOTCHA: npx jest hung (nvm/npx shell quirk) → use `node node_modules/.bin/jest --silent`; killed git mid-write left corrupted index+stale index.lock → rm lock + `git reset` to restore (worktree untouched).
- [x] i18n items celtic SHIPPED (`8138342`): `data/i18n/sv/items-celtic.json` = 25/25 celtic artifacts sv name+description complete:true (Ardagh-kalken, Arthur's Mantel, Pair Dadeni, Caladbolg, Ceridwens kittel, Claddagh-ringen, Claíomh Mór, Cormacs bägare, Dagdas kittel, Dyrnwyn, Excalibur, gröne riddarens yxa+jevne, sjungande harpan, Lia Fáil, Tegau Eurfrons mantel, Nanteos-bägaren, Peredurs graal, Prydwen, sälskinnet, Emains silverkvist, Snettisham-halsringen, Gáe Assail, Tara-broschen, runda bordet). Gate: slug x25 exact-match, desc >=0.6x EN, 0 Cyrillic-е; coverage map now 13 sv keys (items-celtic + items-nordic); shards rebuilt; jest 222/222; no js changes -> sw stays v1_0_26
- [x] i18n Phase 2 Wave H — East Asia SHIPPED (229d92e, moon fix 1b3fc67)
- [x] i18n Wave H Gemini translate polish (uncommitted work rescued + shipped): reverted netlify.toml CSP rewrite (was missing 'unsafe-inline', would have broken inline JSON-LD on all 11 pages; re-added full old CSP + cache headers + Google Translate hosts); removed 3 console.log debug lines from language-toggle (file back to HEAD); translate.js MAX_STRINGS 5→200, CHUNK_SIZE 25, header comment restored; kept good parts: chunked translation w/ per-chunk soft-fallback + toast notification, localStorage persistence (wl:active_gmlang), auto-retranslate on reload via ?gmlang param, translateChrome only translates visible data-i18n keys, netlify function 15→25s timeout; sw v1_0_26; eslint 0 err, jest 222/222
- [x] i18n Phase 2 Wave F SHIPPED Aug 23 (e3f5f3e..T2): batch 4: entries 151-200 (Sirdalsjøormen, Skoffín, Skotta, Skeppsrået, Stormkäringen, Skvadern, Storsjöodjuret-området, Pesta, Draugar/Draugen/Draugr-varianter, Dvärgar m fl) sv name+summary+description complete:true -> creatures-nordic.json = 200 entries / 150 complete; gate passed; pesta browser-verified badge-free; GOTCHA: chunk writes missing name/summary fields -> validation gate caught 12 gaps, fixed in one pass; no js changes -> sw v1_0_23
- [x] i18n Phase 2 Wave E SHIPPED Aug 23 (70dff66..T2): spec/plan wave-e docs — batch 3: entries 101-150 (Guri Kunna, Gråmannen, Grýla x2, Gruvrået, Fenrir, Fossegrimen, Fylgje, Skogsrået, Storsjöodjuret, Stállo, Suliskongen m fl) sv name+summary+description complete:true -> creatures-nordic.json = 150 entries / 100 complete; validation gate passed; fenrir browser-verified badge-free sv desc; no js changes -> sw stays v1_0_23
...
- [x] Perf follow-up (optional): bestiary still ~22MB upfront for grid/facets (perf 0.89 vs 0.90 floor warn) → progressive per-shard loading = bigger refactor, defer until numbers matter
- [x] Perf follow-up SHIPPED (1aa7365): bestiary cold-path split — slim grid index `data/sharded/creatures/index.json` (2.4MB, 6 fields/entry, generator `archive/scripts/build-creatures-index.py` + `tests/creatures-index.test.js` parity gate) replaces 12MB region-shard bulk for grid/facets/search/related; detail opens load full object via `Shimmer.getItem(slug)` (lazy slug batch); stories cross-ref corpus now loads on first detail open instead of page load (`loadIndex` in Shimmer reuses batch overlay decoration with `_slim` flag). Gates: eslint 0, jest 240/240 (20 suites, incl. new loadIndex sv/slim tests + index integrity), chromium e2e bestiary smoke PASS (index fetched, 0 by-region prefetch, 0 stories preload, deep-link detail renders). Deploy zip rebuilt 1497 files.
...
- [x] T8 — Eastern Europe/Baltic/Arctic batch 17xx (~110 items)
  ...
  - [x] T8b — Eastern Europe/Caucasus/Central Asia batch (80 items, 17xx; `c6cfc49`; 561→641; consolidated `batch-17xx-eastern-europe.json` = 26 slavic-west + 33 new-cleaned + 23 central-asia − kannel dup − ia-romanian-blouse→romanian-ie merge; dropped dups kankles/kokle/kannel/laimas-distaff (T8a) + martenitsa/matyo (intra-batch); igil horsehead-fiddle alias removed; tar→tar-azerbaijani id fixed; 20 descs trimmed ≤400; spec-review APPROVE (5 borderline kept: latvian-josta, szekely-kapu HU, kukeri garment, crown-of-saint-stephen archaeological, fujara 2008); shards rebuilt; jest 172/172, chromium 414/414; stale draft-17xx-baltic/new/central-asia files cleaned Aug 21 (76 slugs verified superseded, kannel = estonian-kannel dup))
- [x] T9 — counts/docs refresh (README, .zenodo.json, index stats, about, world) — 319→641 (`1584bec`; jest 172/172, items-viewer+cross-check e2e 45/45)
- [x] T10 — sw.js cache bump v1_0_15 → v1_0_16 (`3ab11ea`)
- [x] T11 — Zenodo v1.2.0 release — **DOI `10.5281/zenodo.21941501`** (record 21941501, concept 21941500; tag `v1.2.0` pushed 2026-08-15, workflow green; zip 350 files/20MB, verified 641 items, desc min 500/median 890, schemas incl.; site/docs updated about.html + README). ~670 items target not reached — closed at 641.
...
- [x] Expansion: all 641 items → 16 input batches (`archive/scripts/expansions/items/expand/batch-01..16-input.json`, 40-41 each); dispatch research agents to write `batch-NN-output.json` (desc ≥500, cross-refs added); verify via `verify-items-expansion.py`; apply via `apply-items-expansion.py` (in-place, tmp+os.replace, backref sync); shards → jest → chromium e2e → commit data/ only. DONE `E2d1` commit: 641/641 verified 0 err/0 warn; applied (desc min 500, median 890, max 1223; 187 creature-ref + 170 story-ref items; +53 creature +37 story backrefs, 0 broken); jest 172/172, chromium 413/414 (1 pre-existing scroll-restore timing flake, passes 3/3 in isolation). Fix: apply-items-expansion.py ROOT needed 4th `..` (was resolving to archive/).
  - [x] **batch-13 (South Asia 1528-1552 + Oceania 1601-1615) output DONE** (Aug 15): 40/40 descs expanded to 740-883 chars; cross-refs added — gata-bera-drum & sanni-yaksha-mask → story the-legend-of-gara-yakka; didgeridoo → rainbow-serpent/julunggul-aboriginal; bullroarer → daramulum-aboriginal; hei-tiki → story the-origin-of-the-tiki; waka-taua → taniwha; mukut/kete keep existing. Applied as part of `3bd1095` (all 16 batches, 641/641).
...
- [x] i18n sv swedish items overlay SHIPPED Sep 7 (`d4e7129`): 41/41 Swedish artifacts full sv name+desc complete:true; shards rebuilt; 15 sv coverage keys; jest 222/222; no js changes -> sw stays v1_0_26.
- [x] i18n sv mediterranean items overlay SHIPPED Sep 7 (`2d147fe`): 38/38 Mediterranean artifacts full sv name+desc complete:true; shards rebuilt; 16 sv coverage keys; jest 222/222; no js changes -> sw stays v1_0_26.
...
- [x] **NO Wave 4 SHIPPED Sep 7**: no-mode browser smoke PASS (/tmp/wl-local, threaded srv :8123 — lang=no + NO titles + HJEM nav x3 pages, menu no=1/sv=1, choose-no→?lang=no, sv regression HEM/lang=sv); gates eslint 0 + jest 226/226 (fast harness /tmp/wl-local); deploy zip rebuilt (/tmp/whispering-lore-deploy.zip, 1496 files, all no+sv overlays included, zip-copy manifest.i18n.no all true).

## Project Health Fixes — Sep 12 (deep-review items; plan: docs/superpowers/plans/2026-09-12-project-health-fixes.md)

- [x] **W1 search index SHIPPED Sep 12** (`02a9946`): archive/scripts/build-search-index.py → data/sharded/search-index.json (4.1MB, 3668 creatures + 2185 stories + 641 items, excerpt 320, slug parity vs masters); tests/search-index.test.js (slug parity + excerpt + <9MB; +`/* eslint-env node */` pragma for eslint no-undef); js/search-viewer.js loadAll() = single index fetch, body prefers item.excerpt, legacy full-shard fallback keeps __FULL_* globals; jest 244/244 (21 suites); eslint 0 errors; search smoke PASS (149 "troll" results, only search-index.json + manifest fetched, 0 console errors).
- [x] **W2 entity prerender + sitemap SHIPPED Sep 12** (tooling only — archive/ gitignored): archive/scripts/seo-build.py (creature|story|item/{slug}/index.html STATIC, JSON-LD + canonical + og + breadcrumb, zero JS; full sitemap.xml 6504 urls, root `<loc>` https://whisperinglore.com/); archive/scripts/build-deploy.sh (git ls-files filter → rsync stage → seo-build → zip). Deploy zip REBUILT /tmp/whispering-lore-deploy.zip = 14,522 files (1499 repo + entity pages), sitemap.xml 712KB, search-index.json + es overlays inside, 0 excluded paths.
- [x] **W3 hreflang x-default SHIPPED Sep 12** (`833d571`): /tmp/x-default-insert.py added x-default after each `en` alternate → 11 pages (404, about, bestiary, index `/`, items, methodology, mylore, quiz, search, stories, world); homepage href = https://whisperinglore.com/; jest 244/244 green.
- [x] **W4 gates/docs/todos SHIPPED Sep 12**: jest 244/244 + eslint 0 (harness); docs/DEPLOYMENT.md += "Zip deploy (local pipeline)" section (build-deploy.sh + seo-build.py + committed-short-sitemap vs deployed-full note + search-index regen note); probe-search.cjs smoke PASS; zip rebuilt with W3 html changes below.
- Deferred (documented in plan docs/superpowers/plans/2026-09-12-project-health-fixes.md): bestiary virtualization, GT 45-lang demotion, analytics, dataset-as-product, repo off iCloud, font subsetting/preload.
