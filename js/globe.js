...
const NIGHT_TEX_SIZE = 1024
const DOT_DEFAULT = 0xdc2626
const DOT_SELECTED = 0xfbbf24
const BORDERS_URL = 'vendor/topojson/countries-110m.json'
let selectedCountry = null
const dotMeshes = []
const countryData = {}

function getCanvas() {
  return document.getElementById('globe-canvas')
}
function getTooltip() {
  return document.getElementById('globe-tooltip')
}
function getInfo() {
  return document.getElementById('globe-country-info')
}
...
function generateNightTexture() {
  ...
  const cities = [
    { lat: 40.7, lng: -74.0, r: 3.5 },
    { lat: 34.1, lng: -118.2, r: 3.0 },
    { lat: 51.5, lng: -0.1, r: 3.0 },
    { lat: 48.9, lng: 2.3, r: 2.5 },
    { lat: 35.7, lng: 139.7, r: 3.5 },
    { lat: 39.9, lng: 116.4, r: 3.0 },
    { lat: 22.3, lng: 114.2, r: 2.5 },
    { lat: 19.4, lng: -99.1, r: 2.0 },
    { lat: -23.6, lng: -46.6, r: 2.5 },
    { lat: 28.6, lng: 77.2, r: 2.5 },
    { lat: 31.2, lng: 121.5, r: 3.0 },
    { lat: 55.8, lng: 37.6, r: 2.5 },
    { lat: 41.0, lng: 28.9, r: 2.0 },
    { lat: 1.3, lng: 103.8, r: 2.5 },
    { lat: 37.6, lng: 127.0, r: 2.5 },
    { lat: 52.5, lng: 13.4, r: 2.0 },
    { lat: -33.9, lng: 151.2, r: 2.0 },
    { lat: -34.6, lng: -58.4, r: 2.0 },
    { lat: 30.0, lng: 31.2, r: 2.0 },
    { lat: 25.2, lng: 55.3, r: 1.5 },
    { lat: 10.8, lng: 106.7, r: 2.0 },
    { lat: 6.5, lng: 3.4, r: 1.5 },
    { lat: -6.2, lng: 106.8, r: 2.0 },
    { lat: 14.6, lng: 121.0, r: 2.0 },
    { lat: 45.5, lng: -73.6, r: 2.0 },
    { lat: 47.6, lng: -122.3, r: 2.0 },
    { lat: 37.8, lng: -122.4, r: 2.5 },
    { lat: 42.4, lng: -71.1, r: 2.0 },
    { lat: 39.0, lng: -77.0, r: 2.0 },
    { lat: 43.7, lng: -79.4, r: 2.0 },
    { lat: 41.9, lng: 12.5, r: 2.0 },
    { lat: 40.4, lng: -3.7, r: 2.0 },
    { lat: 38.7, lng: -9.1, r: 1.5 },
    { lat: 59.3, lng: 18.1, r: 1.5 },
    { lat: 52.4, lng: 4.9, r: 1.5 },
    { lat: 53.3, lng: -6.2, r: 1.5 },
    { lat: 50.1, lng: 14.4, r: 1.5 },
    { lat: 44.8, lng: 20.5, r: 1.0 },
    { lat: 33.5, lng: 36.3, r: 1.0 },
    { lat: 31.8, lng: 35.2, r: 1.0 },
    { lat: 24.7, lng: 46.7, r: 1.5 },
    { lat: 23.0, lng: 72.6, r: 1.5 },
    { lat: 13.8, lng: 100.5, r: 2.0 },
    { lat: 21.0, lng: 105.8, r: 1.5 },
    { lat: 23.1, lng: 113.3, r: 2.0 },
    { lat: 30.6, lng: 114.3, r: 2.0 },
    { lat: 36.1, lng: 120.4, r: 1.5 },
    { lat: 34.7, lng: 135.5, r: 2.5 },
    { lat: 35.4, lng: 139.4, r: 2.0 },
    { lat: 31.0, lng: -9.7, r: 1.0 },
    { lat: -26.2, lng: 28.0, r: 1.5 },
    { lat: -15.8, lng: -47.9, r: 1.5 },
    { lat: 4.6, lng: -74.1, r: 1.5 },
    { lat: 9.9, lng: -84.1, r: 1.0 },
    { lat: -36.8, lng: -73.1, r: 1.0 },
    { lat: -12.0, lng: -77.0, r: 1.5 },
    { lat: 56.9, lng: 24.1, r: 1.0 },
    { lat: 50.5, lng: 30.5, r: 1.5 },
    { lat: 54.7, lng: 25.3, r: 1.0 },
    { lat: 59.4, lng: 24.7, r: 1.0 },
  ]

  cities.forEach(function (c) {
    const x = ((c.lng + 180) / 360) * NIGHT_TEX_SIZE
    const y = ((90 - c.lat) / 180) * (NIGHT_TEX_SIZE / 2)
    const baseR = c.r || 2
    const grad2 = ctx.createRadialGradient(x, y, 0, x, y, baseR * 6)
  ...
  for (let i = 0; i < 200; i++) {
    const rx = Math.random() * NIGHT_TEX_SIZE
    const ry = Math.random() * (NIGHT_TEX_SIZE / 2)
    const rr = 0.5 + Math.random() * 1.5
    const ro = 0.05 + Math.random() * 0.15
    ctx.beginPath()
...
function initGlobe() {
  ...
  const nightTex = new THREE.CanvasTexture(generateNightTexture())
  ...
  const glowMat = new THREE.ShaderMaterial({
    ...
    fragmentShader:
      'varying vec3 vNormal; void main() { float intensity = pow(0.7 - dot(vNormal, vec3(0, 0, 1.0)), 2.5); gl_FragColor = vec4(0.6, 0.1, 0.1, intensity * 0.3); }',
    transparent: true,
    side: THREE.BackSide,
    blending: THREE.AdditiveBlending,
  })
  const glowSphere = new THREE.Mesh(
    new THREE.SphereGeometry(GLOBE_RADIUS + 0.3, 48, 48),
    glowMat
  )
    ...
    dotMeshes.forEach(function (d) {
      ...
      if (d === dot) {
        ...
        if (getInfo()) {
          ...
          const countryCreatures = []
          const countryStories = []
          const ctryLower = countryName.toLowerCase()

          allCreatures.forEach(function (c) {
            if (
              c.country &&
              c.country.toLowerCase().indexOf(ctryLower) !== -1
            ) {
          ...
          allStories.forEach(function (s) {
            if (
              s.country &&
              s.country.toLowerCase().indexOf(ctryLower) !== -1
            ) {
          ...
          let html =
            '<span class="globe-country-name">' +
            window.__sharedUtils.escapeXml(countryName) +
            '</span>' +
            '<span class="globe-country-count">' +
            creatureCount +
            ' creatures documented</span>'
          ...
          if (countryCreatures.length > 0) {
            ...
            countryCreatures.slice(0, 12).forEach(function (c) {
              html +=
                ...
                '" class="globe-creature-link" data-slug="' +
                window.__sharedUtils.escapeXml(c.slug) +
                '">' +
            ...
            if (countryCreatures.length > 12) {
              html +=
                '<a href="bestiary.html?country=' +
                encodeURIComponent(countryName) +
                '" class="globe-view-all">View all ' +
                countryCreatures.length +
                ' &rarr;</a>'
            }
            html += '</div>'
          }
          ...
          if (countryStories.length > 0) {
            ...
            if (countryStories.length > 6) {
              html +=
                '<a href="stories.html?country=' +
                encodeURIComponent(countryName) +
                '" class="globe-view-all">View all ' +
                countryStories.length +
                ' &rarr;</a>'
            }
            html += '</div>'
          }
...
function createDots(countries, scene) {
  ...
  const mat = new THREE.MeshBasicMaterial({ color: DOT_DEFAULT })
  const selectedGeo = new THREE.SphereGeometry(0.08, 12, 12)
  const selectedMat = new THREE.MeshBasicMaterial({ color: DOT_SELECTED })
  ...
  countries.features.forEach(function (f) {
    ...
    const lat = (centroid[1] * Math.PI) / 180
    const lng = (centroid[0] * Math.PI) / 180
    ...
    const dotScale = 1 + Math.min(creatureCount / 20, 4)
...
window.refreshGlobeDots = function () {
  ...
  dotMeshes.forEach(function (dot) {
    ...
    dot.userData.creatureCount =
      creatureCount || Math.floor(Math.random() * 3 + 1)
    const dotScale = 1 + Math.min(creatureCount / 20, 4)
    dot.scale.set(dotScale, dotScale, dotScale)
  })
}
...
window.renderCountryList = function () {
  ...
  if (names.length === 0) {
    container.innerHTML =
      '<p class="loading-text">No country data available</p>'
    return
  }
  let html = ''
  ...
  container.addEventListener('click', function (e) {
    const row = e.target.closest('.globe-country-row')
    if (row) {
      ...
      entries.forEach(function (entry) {
        if (entry.isIntersecting) {
          try {
            ...
            const list = document.getElementById('globe-country-list')
            if (list)
              list.innerHTML =
                '<p style="grid-column:1/-1;text-align:center;color:var(--text-secondary)">Unable to load 3D globe</p>'
          }
          observer.disconnect()
        }
      })
