...
;(() => {
  ...
  const applyFilters = (all) => {
    ...
    const filtered = all.filter((q) => {
      ...
      if (geoValue !== 'all') {
        ...
        if (q.extra.geo.country !== geoValue && q.extra.geo.region !== geoValue)
          return false
      }
      return true
    })
  ...
  const renderNext = async () => {
    ...
    const html = `
      <section class="quiz-question">
        ...
        <p class="quiz-prompt">${window.__sharedUtils.escapeXml(
          question.prompt
        )}</p>
        <div class="quiz-answers">
          ${question.answers
            .map(
              (a, i) =>
                `<button class="quiz-answer" data-index="${i}">${window.__sharedUtils.escapeXml(
                  a.text
                )}</button>`
            )
            .join('')}
    ...
    document.querySelectorAll('.quiz-answer').forEach((btn) => {
      btn.addEventListener('click', () =>
        handleAnswer(
          parseInt(btn.dataset.index, 10) === correctIndex,
          question,
  ...
  const handleAnswer = (isCorrect, q, chosenIndex) => {
    ...
    if (currentLevel === 6 && q.sub_question && asked <= maxQuestions) {
      setTimeout(() => {
        answering = false
        renderSubQuestion(q.sub_question)
      }, 1500)
    } else {
      setTimeout(() => {
        answering = false
    ...
    timerId = setInterval(() => {
      ...
      if (timeLeft <= 0) {
        ...
        document.querySelectorAll('.quiz-answer').forEach((b) => {
          b.classList.add(
            (b.dataset.answer === 'true') === trueIsCorrect
              ? 'correct'
              : 'incorrect'
          )
        })
