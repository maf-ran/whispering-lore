...
test.describe('Items viewer', () => {
  test('shows the full artifact count', async ({ page }) => {
    ...
    await expect(count).toHaveText(new RegExp(String(ITEM_COUNT)), { timeout: 20000 });
  });
  ...
  test('search narrows the artifact grid', async ({ page }) => {
    ...
    await expect(page.locator('.item-count')).toContainText('1 of ' + ITEM_COUNT, { timeout: 10000 });
  ...
  test('index stats show artifact total', async ({ page }) => {
    ...
    await expect(stat).toHaveText(String(ITEM_COUNT), { timeout: 20000 });
  });
});
